...
void process_instruction(vector<string> &contents, unordered_map<string, int> &registers, int &max_val)
{
    ...
    {
        registers[r1] += change;
        max_val = std::max(max_val, registers[r1]);
...
int main()
{
    ...
    while (getline(f, line))
    {
        ...
        process_instruction(contents, registers, max_val);
    }
